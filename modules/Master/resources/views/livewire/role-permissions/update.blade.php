...
<form wire:submit="save" class="card">
    ...
    <div class="px-5 py-4 grid grid-cols-1 gap-x-8 md:grid-cols-2">
        ...
        <div class="form-groups">
            ...
            <input type="text" placeholder="Enter role description" class="text-input" wire:model="form.description">
                ...
                <div class="flex gap-y-4 gap-x-4 flex-wrap items-center md:gap-x-6">
                    @if ($entityKey == 'achievement')
                        @foreach ($achievementPermissions as $key => $value)
                            <label for="{{ $entityKey . '-' . $key }}" class="flex items-center gap-x-2 cursor-pointer">
                                <div class="w-5 h-5 relative">
                                    <input id="{{ $entityKey . '-' . $key }}" type="checkbox"
                                        value="{{ $key . ' ' . $entityKey }}"
                                        class="peer appearance-none w-full h-full bg-gray-200 rounded-sm focus:ring-brand-500 focus:ring-1 checked:bg-brand-500" wire:model="form.permissions">
